...
export class AGPortal extends AGObject {
    ...
    onCollisionEnter(obj) {
        ...
        if ((obj.type === "PLAYER" || obj.tag === "ENEMY") && !this._blockedObjects.includes(obj) && evaluateAll(this._conditions)) {
            ...
            if (this._exit != null) this._exit.blockedObjects.push(obj);
            if (this._exit != null) obj.position = this._exit.position.clone();
            if (this._exit != null) console.log("[AGPortal] Teleporting Object: " + obj.name + " to exit: " + this._exit.name);
