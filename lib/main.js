...
let area = new AGGameArea("ebene", new Vector3(30, 2.5, 10));
let areaID = getIdByReference(area);

let room_1 = new AGRoom("Erster Raum", new Vector3(10.0, 2.5, 10.0), new Vector3(5.0, 0.0, 5.0), areaID);
...
let gegner_ss = new AGSoundSource("schritte", "sounds/steps.wav", true, 1, room_1ID);
let player = new AGPlayer("spieler", new Vector3(8.0, 1.0, 5.0), new Vector3(1, 0, 0), new Vector3(1, 1, 1));
let player_hit = new AGSoundSource("hit", "sounds/ouch.mp3", false, 1, room_1ID);
...
let exit1 = new AGRoomExit("Ausgang", new Vector3(5.0, 1.0, 1.0), new Vector3(1, 0, 0), new Vector3(1, 1, 1));

let gegnerID = getIdByReference(gegner);
let gegner_ssID = getIdByReference(gegner_ss);
let playerID = getIdByReference(player);
let player_hitID = getIdByReference(player_hit);
let door1ID = getIdByReference(door1);
let door2ID = getIdByReference(door2);
let exit1ID = getIdByReference(exit1);
...
let keyID = getIdByReference(key);
