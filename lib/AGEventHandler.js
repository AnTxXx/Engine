...
export class AGEventHandler {
    ...
    */
    ...
    findEventsAfterCall(object, action) {
        let events = [];
        for (let i = 0; i < this._events.length; i++) {
            if (this._events[i].object === object && this._events[i].action === action) events.push(this._events[i]);
        }
        return events;
    }

    call(object, action) {
        let events = this.findEventsAfterCall(object, action);
